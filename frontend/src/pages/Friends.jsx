...
const Friends = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const userResponse = await axios.get(`${window.location.origin}/api/v1/user-info`, {
          headers: {
        ...
        if (friendsIds.length > 0) {
          const friendsResponse = await axios.post(`${window.location.origin}/api/v1/friends-info`, {
            ids: friendsIds
          }, {
            headers: {
              id: localStorage.getItem('id'),
  ...
  const handleSearch = async (event) => {
    ...
    try {
      const response = await axios.get(`${window.location.origin}/api/v1/search-player`, {
  ...
  const addFriend = async (friendId) => {
    try {
      const response = await axios.put(`${window.location.origin}/api/v1/add-friend`, {
        friendId
      }, {
        headers: {
          id: localStorage.getItem('id'),
